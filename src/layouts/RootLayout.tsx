...
function GlobalBottomNavigation() {
  ...
  /** ≈‘⁄«— «·√ÌﬁÊ‰… «·Œ«—ÃÌ… ó Ìı’›¯Û— ⁄‰œ › Õ «·ﬁ«∆„…° Ì‘ ⁄· ⁄‰œ —”«·… ÃœÌœ… */
  ...
  /** ›ﬁ«⁄«  «·‰ﬁ— ó „—… Ê«Õœ… ⁄‰œ «·÷€ÿ À„  ı“«·  ·ﬁ«∆Ì« (·«   ﬂ—— ﬂ· À«‰Ì…) */
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      // ·« ‰ı‘⁄· «·√ÌﬁÊ‰… Â‰« ó ›ﬁÿ ⁄‰œ “Ì«œ… «·⁄œ«œ (useEffect √œ‰«Â)
    };
    refresh();
    // √Ê·  Õ„Ì·: ≈‰ ÊıÃœ  —”«∆· €Ì— „ﬁ—Ê¡… √ŸÂ— «·≈‘⁄«—
    ...
    if (initialTotal > 0) {
      prevUnreadRef.current = 0; // Õ Ï Ì· ﬁÿ useEffect «·“Ì«œ…
      setCompanyUnreadTotal(initialTotal);
    }

    // Ã·» —”«∆· Ê«—œ… „‰ «·‹ API ≈‰ ÊıÃœ  Êœ„ÃÂ«
    let cancelled = false;
    const poll = async () => {
      try {
        const endpoints = [
  ...
  function deleteUserPeer(peerId: string) {
    if (!user?.id) return;
    if (!window.confirm('Õ–› Â–Â «·„Õ«œÀ… „‰ ﬁ«∆„ ﬂø ·‰  ıÕ–› —”«∆· «·‘«  ‰›”Â« ≈·« ≈–« „”Õ Â« „‰ œ«Œ· «·„Õ«œÀ….')) return;
  ...
  // ‘«  «·œ⁄„ / Privacy / √Ì ‘«‘…  ÿ·» ≈Œ›«¡ «·‘—Ìÿ ⁄»— «·ÕœÀ √Ê ﬂ·«” body
  ...
  // Ìı»œ√ „‰ «·—«»ÿ (?openTextPosts=1) √Ê class ⁄·Ï body ›ﬁÿ ó ·« ‰› —÷ true ·ﬂ· /add-friend
  // Õ Ï ·« ÌŒ ›Ì «·‘—Ìÿ «·”›·Ì ÊÌ ⁄ÿ¯· «· ‰ﬁ· ··ÂÊ„/‘« /·«Ì›.
  ...
  // „“«„‰… „‰ «·—«»ÿ ÿ«·„« «·⁄·«„… „ÊÃÊœ… ( ıÕ–› »⁄œ «” Â·«ﬂÂ« ›Ì add-friend)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // class ⁄·Ï body √ÊÀﬁ „‰ «·ÕœÀ ÊÕœÂ ⁄‰œ «·Œ—ÊÃ/«·œŒÊ· ·· ÿ»Ìﬁ
    const obs = new MutationObserver(syncFromBodyClass);
    obs.observe(document.body, { attributes: true, attributeFilter: ['class'] });
    syncFromBodyClass();
    return () => {
  ...
  // ﬁ«∆„… «·‘«  ‰ıﬁ·  œ«Œ· √ÌﬁÊ‰… «·√’œﬁ«¡ ( »ÊÌ» Friends / Chat)

  // ≈Œ›«¡ «·‘—Ìÿ  ·ﬁ«∆Ì« œ«Œ· √Ì ‘«  (›—œÌ √Ê ”—Ì √Ê œ⁄„)° √Ê ⁄‰œ › Õ ‰«›–… ﬂ «»… „‰‘Ê— ÃœÌœ°
  // √Ê ⁄‰œ › Õ ‘Ì  "≈—”«· «·„‰‘Ê— ≈·Ï «·√’œﬁ«¡" ó Ê≈—Ã«⁄Â  ·ﬁ«∆Ì« ⁄‰œ «·Œ—ÊÃ „‰ ﬂ· Õ«·…
  // ≈Œ›«¡ «·‘—Ìÿ «·”›·Ì √Ì÷« √À‰«¡ ’›Õ… «·»Ê”  «·‰’Ì
  ...
  const miniChatOverlay = miniChat && user ? (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: '#eab308', fontWeight: 800 }}>{miniChat.name || miniChat.username || '„Õ«œÀ…'}</div>
          <div style={{ color: 'rgba(150,200,200,0.5)', fontSize: 12 }}>‘«  «·„‘«—ﬂ… ∑ ‘—ﬂ… / „” Œœ„</div>
        </div>
      </div>
          ...
          <div style={{
            ...
            {(miniChat.postText || (miniChat.mediaItems && miniChat.mediaItems.length)) ? (
              <div style={{ padding: 12, borderBottom: miniChat.note ? '1px solid rgba(234,179,8,0.25)' : 'none' }}>
                <div style={{ color: '#eab308', fontSize: 12, fontWeight: 800 }}>«·„‰‘Ê— / «·„‰ Ã</div>
                {(miniChat.mediaItems || []).map((m, i) => {
                  ...
                  if (ty === 'pdf') return <a key={i} href={m.url} target="_blank" rel="noreferrer" style={{ display: 'block', margin: '8px 0', color: '#00BCD4', fontWeight: 800 }}>› Õ „·› PDF</a>;
            ...
            {miniChat.note ? (
              <div style={{ padding: 12 }}>
                <div style={{ color: '#eab308', fontSize: 12, fontWeight: 800 }}>«· ⁄·Ìﬁ / ¬Œ— —”«·…</div>
      ...
      >
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: 0, color: '#00BCD4', fontWeight: 900, fontSize: '0.95rem' }}>‘«  «·⁄„·«¡</p>
          ...
          {companyInbox.length === 0 && (
            <div style={{ textAlign: 'center', marginTop: 56, padding: '0 20px' }}>
              ...
              <p style={{ color: 'rgba(150,190,190,0.55)', fontSize: '0.75rem', margin: 0, lineHeight: 1.5 }}>
                ⁄‰œ «” ›”«— √Ì „” Œœ„ ⁄‰ „‰ Ã ” ŸÂ— «·„Õ«œÀ«  Â‰«  Õ  »⁄÷.
              </p>
            </div>
          )}
          {companyInbox.map(peer => {
            const hasUnread = (peer.unread || 0) > 0;
            const label = peer.name || (peer.username ? `@${peer.username}` : '„” Œœ„');
        ...
        <div style={{
          ...
          {userChatTab === 'friends' && userFriendChats.length === 0 && (
            <div style={{ textAlign: 'center', marginTop: 40, padding: '0 20px' }}>
              <p style={{ color: 'rgba(200,230,230,0.85)', fontSize: '0.88rem', fontWeight: 600 }}>·« „Õ«œÀ«  √’œﬁ«¡ »⁄œ</p>
              <p style={{ color: 'rgba(150,190,190,0.55)', fontSize: '0.75rem' }}>⁄‰œ „‘«—ﬂ… „‰‘Ê— ·’œÌﬁ  ŸÂ— «·„Õ«œÀ… Â‰«.</p>
            </div>
          )}
          {userChatTab === 'friends' && userFriendChats.map(peer => {
            const hasUnread = (peer.unread || 0) > 0;
            const label = peer.name || (peer.username ? `@${peer.username}` : '’œÌﬁ');
            return (
                ...
                <button type="button" onClick={() => openFriendPeerChat(peer)} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0, textAlign: 'right' }}>
                    ...
                    <p style={{ margin: '4px 0 0', color: 'rgba(150,190,190,0.7)', fontSize: '0.74rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {peer.lastMessage || '„‘«—ﬂ… „‰‘Ê—'}
                    </p>
                  </div>
                </button>
                <button type="button" title="Õ–›" onClick={(e) => { e.stopPropagation(); deleteFriendPeer(peer.id); }}
          ...
          {userChatTab === 'company' && userProductChats.map(peer => {
            const hasUnread = (peer.unread || 0) > 0;
            const label = peer.name || (peer.username ? `@${peer.username}` : '‘—ﬂ…');
            const timeStr = peer.at
              ? new Date(peer.at).toLocaleString('ar-KW', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })
              : '';
            return (
  ...
  // „” ÿÌ· »”Ìÿ »ÕÃ„ «·√ÌﬁÊ‰… ›ﬁÿ ó »œÊ‰ Ê„Ì÷/ ÊÂÃ ﬂ»Ì—
    ...
    <div style={{
      ...
      {homeCallPhase === 'animating' && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14 }}>
          ...
          <p style={{ color: '#00BCD4', fontWeight: 800, margin: 0 }}>Ã«—Ì «·œŒÊ· ··„ﬂ«·„…Ö</p>
        </div>
      )}
        ...
        >
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {homeCallFriends.length === 0 && (
              <p style={{ color: 'rgba(180,210,210,0.7)', textAlign: 'center' }}>·« √’œﬁ«¡ »⁄œ</p>
            )}
            {homeCallFriends.map(f => {
              ...
              return (
                <button key={f.id} type="button" onClick={() => setHomeCallSelected(s => ({ ...s, [f.id]: !s[f.id] }))} style={{
                  ...
                  <span style={{ flex: 1, fontWeight: 700 }}>{f.name || f.username || '’œÌﬁ'}</span>
                  <span style={{
      ...
      {(homeCallPhase === 'connecting' || homeCallPhase === 'live') && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, direction: 'ltr' }}>
            <button type="button" onClick={() => setHomeCallMembersOpen(o => !o)} style={{ width: 40, height: 40, borderRadius: '50%', border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#111' }} aria-label="«·„ÊÃÊœÊ‰">
              <Users size={18} />
            </button>
            <div style={{ textAlign: 'center', flex: 1, padding: '0 8px' }}>
              <p style={{ margin: 0, fontWeight: 800, color: '#111', fontSize: 16 }}>
                {(() => {
                  ...
                  const hostName = me?.name || me?.username || '√‰ ';
                  if (!joinedOthers.length) return hostName;
                  return [hostName, ...joinedOthers.map(m => m.name || m.username || '„” Œœ„')].join(' ∑ ');
                })()}
              </p>
              <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>{homeCallPhase === 'live' ? '„ﬂ«·„… ’Ê Ì…' : 'Ã«—Ì «·« ’«·Ö'}</p>
            </div>
            <span style={{ width: 40 }} />
          </div>
            ...
            <div style={{
              ...
              {homeCallMembers.map(m => (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 4px', direction: 'rtl' }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#111', flex: 1, textAlign: 'right' }}>{m.name || m.username || '„” Œœ„'}</span>
                  <span style={{ fontSize: 12, color: '#4b5563', flexShrink: 0 }}>{m.joined ? '›Ì «·„ﬂ«·„…' : 'Ìıœ⁄ÏÖ'}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button type="button" onClick={toggleHomeCallSpeaker} aria-label="«·”„«⁄…" style={{
              ...
              <button type="button" onClick={toggleHomeCallMute} aria-label="ﬂ „" style={{
                width: 52, height: 52, borderRadius: '50%', border: 'none',
              ...
              <button type="button" onClick={() => setHomeCallEmojiOpen(o => !o)} aria-label="≈Ì„ÊÃÌ" style={{
                width: 52, height: 52, borderRadius: '50%', border: 'none',
                background: '#eceff1',
                cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
              }}>
              ...
              <button type="button" onClick={() => { void leaveHomeGroupCall(); }} aria-label="≈‰Â«¡ «·„ﬂ«·„…" style={{
                width: 52, height: 52, borderRadius: '50%', border: 'none',
                background: '#e11d48',
                cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
              }}>
  ...
  <nav aria-label="Main navigation" style={{
    ...
    borderTop: '1px solid rgba(0,188,212,0.12)',
      ...
      position: 'relative',
        ...
        {/* Plus menu ó Settings / Friends / Account live / Text posts radar */}
        <div style={{
          position: 'absolute',
          right: 8,
          top: '50%',
...
export default function RootLayout({
  ...
  // ’›Õ«  "„·¡ «·‘«‘…" „À· «·‘«  «·›—œÌ (/chat)  œÌ— «— ›«⁄Â« Ê”ﬂ—Ê·Â« «·œ«Œ·Ì
  // »‰›”Â« (height: 100dvh + overflow: hidden) Ê ıŒ›Ì «·‘—Ìÿ «·”›·Ì √’·«.
  // ·Ê √»ﬁÌ‰« «·‹ padding-bottom «·„ÕÃÊ“ ·‘—Ìÿ «· ‰ﬁ·° Ì’Ì— «— ›«⁄ «·„Õ ÊÏ
  // «·ﬂ·Ì √ÿÊ· „‰ «·‘«‘…° › ’Ì— «·’›Õ… ﬁ«»·… ··”ﬂ—Ê· —€„ √‰Â« Ì› —÷  ﬂÊ‰
  // À«» … ó Ê√Ì ”ﬂ—Ê·  ·ﬁ«∆Ì («” —Ã«⁄ „Ê÷⁄ «·”ﬂ—Ê·°  ’€Ì— ‘—Ìÿ ⁄‰Ê«‰
  // «·„ ’›Õ° ≈·Œ) Ì”Õ» «·ÂÌœ— «·„· ’ﬁ ›Êﬁ «·„Õ«œÀ… Œ«—Ã ‰ÿ«ﬁ «·—ƒÌ….
  // ·–·ﬂ ‰·€Ì Â–« «·ÕÃ“  ÕœÌœ« »Â–Â «·’›Õ« .
