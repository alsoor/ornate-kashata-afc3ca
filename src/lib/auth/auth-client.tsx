...
function useStaleSessionRecovery(error: unknown, isPending: boolean, isAuthenticated: boolean): void {
  useEffect(function staleSessionRecovery() {
    ...
    // AbortError is a normal cancellation (e.g. component unmount, preview
    // iframe navigation) — not a stale session. Only recover on genuine errors.
...
export function useSession() {
  // BetterAuth's useSession makes a fetch call internally. During SSR
  // (renderToString) there is no real browser fetch context, which can throw
  // NotSupportedError. We call the hook unconditionally (Rules of Hooks) but
  // suppress its output on the server so the SSR render always returns a safe
  // "pending / not authenticated" state.
  const {
    data: session,
    isPending,
    error
  ...
  const isAuthenticated = !isServer && !isPending && !!session?.user;
  useStaleSessionRecovery(isServer ? null : error, isServer ? true : isPending, isAuthenticated);
  if (isServer) {
    return {
