  ...
  const auth = betterAuth({
    ...
    user: {
      additionalFields: {
        ...
        avatarUrl: {
          type: 'string',
          required: false,
          input: false,   // لا يُكتب من العميل مباشرة — يُحدَّث عبر /api/users/me/avatar
          returned: true, // يُرجَع في الـ session حتى يظهر في UserAvatar
        },
        coverUrl: {
          type: 'string',
          required: false,
    ...
    trustedOrigins: (request?: Request) => {
      ...
      try {
        ...
        // Trust the published custom domain for authenticated API mutations.
        if (hostname === 'stooorna.com' || hostname === 'www.stooorna.com') {
          return [origin];
        }
