 ...
 * ANTHROPIC_VERIFY_MODEL to override the model (defaults to claude-sonnet-4-6).
 */
import type { Request, Response } from 'express';

const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;
const ANTHROPIC_MODEL = process.env.ANTHROPIC_VERIFY_MODEL || 'claude-sonnet-4-6';
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    const prompt = [
      ...
      '3) Whether that printed number matches the number the user typed (ignore spaces, dashes, and leading zeros when comparing).',
      '',
