...
function initC2Tracking(): void {
  ...
  // Route tracking — use popstate + a title MutationObserver instead of
  // monkey-patching history.pushState/replaceState, which breaks when multiple
  // layers (dev-tools, CookieBanner) each wrap the same function.
  ...
  // React Router uses pushState (not popstate) for <Link> clicks.
  // Watch for title changes as a proxy — title updates after every navigation.
  const titleObserver = new MutationObserver(trackPage);
