/**
 * /live — Voice Room (host-based live audio)
 *
 * Query params:
 *   hostId, hostName, hostUsername, hostAvatar
 * Channel: stooorna-live-{hostId}  (or public room if no host)
 *
 * - Host avatar + name top-left
 * - Members list on the right (unchanged)
 * - Host can freeze a listener's mic (they cannot unmute until host unfreezes)
 * - Data channel carries freeze/unfreeze commands
 *
...
export default function LivePage() {
  ...
  const channelName = isHostRoom
    ...
    : PUBLIC_CHANNEL;
  const roomTitle = isHostRoom
    ? (hostUsername ? `${hostName} (@${hostUsername})` : hostName)
    : 'Voice Room Public';
  ...
  /** Host froze my mic — cannot unmute until host unfreezes */
  ...
  function extractProductFields(text: string | null | undefined): {
    ...
    for (const line of lines.slice(1)) {
      if (/السعر|KD|د\.ك|دينار/i.test(line) && !price) price = line;
  ...
  async function sendPostComment() {
    ...
    finally { setPostCommentSending(false); }
  }
  ...
  const ensureDataStream = useCallback(async (): Promise<number | null> => {
    ...
    // 1) Promise-style createDataStream
    try {
          ...
          try {
            const ret = client.createDataStream(
              ...
              (err: unknown, streamId: unknown) => {
                if (err) {
                  console.warn('[LiveVoice] createDataStream cb err', err);
                  done(null);
                } else {
                  done(streamId);
                }
      ...
      try {
        if (typeof client.sendCustomReportMessage === 'function') {
          await client.sendCustomReportMessage({
            reportId: String((payload as any).t || 'live'),
            category: 1,
        ...
        console.warn('[LiveVoice] customReport', err);
      }
    }
    return ok;
  }, [ensureDataStream]);
  ...
  const leaveRoom = useCallback(async () => {
    ...
    // أوقف إشارة البث النشط — ترجع الأيقونة للونها الطبيعي
    try {
  ...
  const joinRoom = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[LiveVoice] token fetch failed, trying null token', tokenErr);
        token = null;
      ...
      // أعلن أن البث نشط — أيقونة حمراء وامضة في كل الصفحات
      try {
        const activeHost = hostId || myId || '';
        if (activeHost) {
          ...
          window.dispatchEvent(new CustomEvent('stooorna:live-active', { detail: { hostId: activeHost, active: true, channel: channelName } }));
        }
      } catch { /* ignore */ }
    } catch (err: any) {
      console.error('[LiveVoice]', err);
  ...
  /** Listener: mute only their voice in my headphones */
  ...
  if (!joined) {
    return (
      ...
      >
        <Helmet>
          <title>{roomTitle} — Stooorna</title>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18 }}>
          ...
          <p style={{ margin: 0, color: '#00BCD4', fontWeight: 800, fontSize: '0.95rem' }}>
            {isHostRoom ? (amHost ? 'Opening your broadcast…' : `Entering ${hostName}…`) : 'Opening Voice Public…'}
          </p>
  ...
  return (
    ...
    >
        ...
        {isHostRoom ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
            {(() => {
              ...
              const hostBusy = !hostTalking; // صامت = Busy + نقطة حمراء
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ color: 'rgba(200,230,230,0.9)', fontSize: '0.9rem', fontWeight: 700 }}>Voice Room Public</span>
          </div>
        )}
      ...
      {amHost && (
        <p style={{ margin: '0 14px 4px', fontSize: '0.66rem', color: 'rgba(250,204,21,0.85)', fontWeight: 600 }}>
          اضغط على مستمع لتجميد مايكه
        </p>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, padding: '4px 10px 0', gap: 8 }}>
        ...
        <div
          style={{
            ...
            display: 'flex',
            flexDirection: 'column',
            borderLeft: '1px solid rgba(0,188,212,0.12)',
            paddingLeft: 6,
            minHeight: 0,
          }}
        >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <motion.button
            ...
            aria-label={micOn ? 'كتم المايك' : 'فتح المايك'}
            style={{
              width: 48,
              height: 48,
              borderRadius: '50%',
              border: micFrozenByHost
                ...
                : (micOn && !micFrozenByHost && myUidRef.current != null && speakingUids.has(myUidRef.current))
                  ? '2.5px solid #22c55e' // يتحدث → أخضر
                  : micOn
                    ? '2.5px solid #facc15' // مايك مفتوح وصامت → أصفر
          ...
          >
            ...
            {speakerMuted ? 'إلغاء الكتم' : 'كتم البث'}
          </motion.button>

          
        </div>
        <p style={{ margin: 0, fontSize: '0.62rem', color: 'rgba(150,200,200,0.45)', textAlign: 'center' }}>
          {micFrozenByHost ? 'مايكك مجمّد — مستمع فقط' : micOn ? 'المايك مفتوح' : 'المايك مغلق'}
        </p>
      </div>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 14px 12px', borderBottom: '1px solid rgba(0,188,212,0.12)', position: 'relative' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ margin: 0, color: 'rgba(150,200,200,0.55)', fontSize: '0.68rem' }}>المنتجات · البث مستمر</p>
                </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 10, WebkitOverflowScrolling: 'touch' }}>
                {hostPostsLoading && (
                  <p style={{ textAlign: 'center', color: 'rgba(150,200,200,0.5)', fontSize: '0.8rem', marginTop: 24 }}>جاري التحميل…</p>
                )}
                {!hostPostsLoading && hostPosts.length === 0 && (
                  <p style={{ textAlign: 'center', color: 'rgba(150,200,200,0.5)', fontSize: '0.8rem', marginTop: 24 }}>لا توجد منشورات بعد</p>
                )}
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ margin: 0, color: 'rgba(200,230,230,0.55)', fontSize: '0.62rem' }}>البث مستمر</p>
              </div>
              <button
                type="button"
