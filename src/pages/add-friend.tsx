...
export default function AddFriendPage() {
  ...
  useEffect(() => {
    ...
    const openComposerFromNav = () => {
      ...
      // الأفراد: بدون New Post — فتح الفيد فقط
      if (!isCompanyUserAccount(user, companies)) {
        setTextPostsPageOpen(true);
        setTextPostsMenuOpen(false);
        return;
  ...
  return <>
      ...
      fontFamily: 'var(--font-sans)'
        ...
        <div className="sticky top-0 z-20" style={{
          ...
          {/* ── ثلاث نقاط مكان أيقونة الأصدقاء: أصدقاء + شات + مايك عام ── */}
          <div style={{
            position: 'absolute', top: 44, right: 14, zIndex: 9,
            opacity: headerOpen ? 1 : 0,
            pointerEvents: headerOpen ? 'auto' : 'none',
            transition: headerOpen ? 'opacity 240ms ease-out 200ms' : 'opacity 140ms ease-in',
          }}>
            <style>{`
              @keyframes stooornaDotPulseG {
                0%,100% { opacity: 0.55; transform: scale(0.85); }
                50% { opacity: 1; transform: scale(1.15); }
              }
              @keyframes stooornaDotPulseW {
                0%,100% { opacity: 0.5; transform: scale(0.85); }
                50% { opacity: 1; transform: scale(1.15); }
              }
              @keyframes stooornaDotPulseR {
                0%,100% { opacity: 0.55; transform: scale(0.85); }
                50% { opacity: 1; transform: scale(1.15); }
              }
            `}</style>
            <motion.button whileTap={{ scale: 0.88 }} type="button"
              onClick={() => setStoryMoreOpen(v => !v)}
              aria-label="المزيد"
              style={{
                width: 22, height: 36, background: 'none', border: 'none',
                color: 'inherit', cursor: 'pointer', padding: 0,
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4,
              }}>
              {(() => {
                let unread = false;
                try { unread = !!isCompanyPublisher && localStorage.getItem('stooorna_company_inbox_unread') === '1'; } catch { unread = false; }
                const c = unread ? '#facc15' : null;
                return (
                  <>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: c || '#22c55e', boxShadow: `0 0 6px ${unread ? 'rgba(250,204,21,0.9)' : 'rgba(34,197,94,0.8)'}`, animation: 'stooornaDotPulseG 1.4s ease-in-out infinite' }} />
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: c || '#ffffff', boxShadow: `0 0 6px ${unread ? 'rgba(250,204,21,0.9)' : 'rgba(255,255,255,0.7)'}`, animation: 'stooornaDotPulseW 1.4s ease-in-out 0.18s infinite' }} />
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: c || '#ef4444', boxShadow: `0 0 6px ${unread ? 'rgba(250,204,21,0.9)' : 'rgba(239,68,68,0.8)'}`, animation: 'stooornaDotPulseR 1.4s ease-in-out 0.36s infinite' }} />
                  </>
                );
              })()}
            </motion.button>
            {storyMoreOpen && (
              <div style={{
                ...
                {/* Settings نُقلت إلى قائمة الثلاث خطوط في الشريط السفلي */}
                {!isCompanyPublisher && (
                <button type="button" onClick={() => { setStoryMoreOpen(false); navigate('/live'); }}
                  style={{
                    width: '100%', display: 'flex', alignItems: 'center', gap: 8,
                    padding: '10px 12px', border: 'none', borderRadius: 8, cursor: 'pointer',
                    background: 'transparent', color: '#ef4444', fontWeight: 800, fontSize: '0.82rem',
                  }}>
                  <Mic size={15} /> Voice Public
                </button>
                )}
              </div>
            )}
